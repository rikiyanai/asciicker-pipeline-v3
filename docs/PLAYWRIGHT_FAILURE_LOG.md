...
### Regression Note — Block Idle Sheets Had Duplicated Rows And Mirrored Blocks (2026-05-11)

User review rejected the redone block idle sheets after opening them in the
workbench.

Confirmed failure:

- Each angle row was the same OCR block, so the "angle" animation did not show
  different facings.
- Each frame had two visible blocks because the generator mirrored the source
  into projection column 1.
- The generated side/corner readability was poor; the expected `/` and `\`
  side glyphs were not present as explicit corner/edge indicators.

Root cause:

- `extract_blocks_idle_redone.py` treated a single extracted screenshot object
  as a complete eight-angle sprite and repeated it across all rows.
- It also filled both engine projection columns with visible content. That is
  valid-looking metadata, but wrong for the temporary "idle player skin" review
  purpose.

Correction:

- Added `scripts/rebuild_block_angle_skins.py`.
- Rebuilt `sprites/blocks_idle_redone/block_*_idle.xp` in place.
- Projection 0 now contains the authored block angles; projection 1 is blank
  instead of a duplicated/mirrored visible block.
- Each XP now has 8 distinct angle rows derived from the OCR base frame with
  explicit `/` and `\` edge glyphs.
- The manifest records `distinct_angle_rows`, `slash_count`,
  `backslash_count`, and the blank projection contract.

Verification:

- `PYTHONPATH=src python3 scripts/rebuild_block_angle_skins.py` rebuilt 65
  files.
- Direct validator: 65 files, no failures; sample
  `block_01_idle.xp` has 8 distinct angle rows, 240 `/` glyphs, 240 `\`
  glyphs, and 0 nonzero cells in projection 1.
- `PYTHONPATH=src python3 -m pytest tests/test_block_angle_skins.py -q` ->
  PASS.
