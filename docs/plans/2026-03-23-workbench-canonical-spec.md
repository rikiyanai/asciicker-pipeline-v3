...
| BUG-11 | G-BUNDLE deterministic Skin Dock readiness path often never reaches playable state | OPEN | In 56 logged G-BUNDLE runs (2026-03-18 through 2026-03-25), the Skin Dock playable-state probe succeeded in only 12 runs. Failure pattern is consistent: `wasmReady: false`, `rafCount: 0`, `overlayVisible: true`, `renderStage: 0` across the full 30-second probe window — WASM never initializes and the overlay never dismisses. G-RANDOM (different authoring path, separate browser session) can pass Skin Dock, but does not clear G-BUNDLE. Root cause not yet isolated; likely a runner/runtime-lane issue (resource loading, iframe lifecycle, or probe timing in `run_bundle_fidelity_test.mjs:877-972`), not an editor/whole-sheet bug. G-BUNDLE is currently not reliable enough to serve as a deterministic closeout gate until this is fixed. Distinct from BUG-10 (button gating). |
...
|------|--------|---------------|----------------|
| G-BUNDLE | `run_bundle.sh` | Deterministic 3-action bundle (idle/attack/death) passes with fidelity + Skin Dock playable + 10s runaround 0 crashes | Regression — fixed inputs. **Currently unreliable due to BUG-11** — Skin Dock playable-state probe fails in the majority of runs. Not a clean closeout gate until BUG-11 is resolved. |
