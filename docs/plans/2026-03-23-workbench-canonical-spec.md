...
| Sprite-by-sprite source-to-frame drag is still only partially proven | `web/workbench.js`, `scripts/xp_fidelity_test/run_source_to_grid_workflow_test.mjs`, `PLAYWRIGHT_FAILURE_LOG.md` fix-attempt entries dated `2026-04-17` | Headed proof now exists for both single manual committed boxes and single auto-detected `Find Sprites` boxes dragged into `9A`. The remaining gap is broader grouped row/column multi-box drag coverage, so the authoring lane is improved but still partial. |
