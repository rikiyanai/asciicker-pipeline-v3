...
2. **Registry roles are mostly wired in live backend execution, but `UQ-004` is still not fully closed.** `config/template_registry.json` is now the live authority for template-driven backend bundle/session/export gates, `/api/workbench/templates` now surfaces `registry_status`, and legacy session identity now resolves `filename_prefix` / `skin_family` on read plus next-save persistence. The remaining open second-authority drift is the hardcoded classic/runtime AHSW range maps (`web/workbench.js:FAMILY_W_RANGE`, `src/pipeline_v2/service.py:_FAMILY_W_RANGE`), which still duplicate registry `ahsw_range`.
...
| Template registry is normalized and mounted-aware in data, and template-driven backend gates now consume it, but classic/runtime override naming still duplicates registry truth | `config/template_registry.json`, `src/pipeline_v2/service.py:1105-1232`, `src/pipeline_v2/service.py:1417-1460`, `src/pipeline_v2/service.py:3018-3178`, `src/pipeline_v2/service.py:3895-3982`, `web/workbench.js:24-40`, `src/pipeline_v2/service.py:79-96` | `ENABLED_FAMILIES` is gone from live backend gates, backend template authorization now enforces the same template-set scope/linkage checks as the browser, and legacy session identity resolves normalized fields on read/save. The remaining `UQ-004` drift is the hardcoded `FAMILY_W_RANGE` / `_FAMILY_W_RANGE` maps, which still duplicate registry `ahsw_range` instead of deriving override naming from the normalized registry. |
...
| Browser bundle scope and backend template-driven authority now consume the same registry contract, but queue closure still waits on the last second-authority cleanup | `src/pipeline_v2/app.py:389-393`, `web/workbench-template-gating.js`, `web/workbench.js:7286-7307`, `src/pipeline_v2/service.py:1105-1232`, `tests/test_template_registry_schema.py`, `tests/web/workbench-template-gating.test.js` | Browser action gating and backend template-driven gating now both honor `skin_family_scope`, `proof_only`, `authorable`, and `template_actions` linkage. `UQ-004` remains open because classic/runtime override-name generation still keeps hardcoded `ahsw_range` truth outside the registry. |
| `UQ-004` registry stabilization backlog is now narrowed to load-path residuals and hardcoded override-name maps | `web/workbench.js:24-40`, `web/workbench.js:7286-7307`, `src/pipeline_v2/app.py:389-393`, `src/pipeline_v2/service.py:79-96`, `src/pipeline_v2/service.py:1174-1232`, `tests/test_template_registry_schema.py` | Registry load/fetch failures are now operator-visible in both API and browser UI, and legacy session identity resolves normalized fields on read/save. Remaining residuals are: empty-registry fallback caching still exists for missing/malformed config, `preview_xp` still falls back to `l0_ref`, and classic/runtime override-name generation still duplicates registry `ahsw_range` via hardcoded maps. |
...
| Registry authority | `src/pipeline_v2/service.py::create_bundle()`, `workbench_create_blank_session()`, `bundle_action_run()`, `workbench_export_bundle()`, `workbench_web_skin_bundle_payload()`, `web/workbench.js`, `src/pipeline_v2/service.py` | Template-driven backend bundle/session/export/runtime gates now derive from the normalized registry, but classic/runtime override-name generation still duplicates registry `ahsw_range` through `FAMILY_W_RANGE` / `_FAMILY_W_RANGE` | `UQ-004` |
...
| UQ-004 | ACTIVE | Finish normalized-registry authority closeout | UQ-002 pass condition met | Preserve the landed backend helper/tests/operator-visibility work, then remove the last hardcoded classic/runtime `ahsw_range` duplicates (`FAMILY_W_RANGE`, `_FAMILY_W_RANGE`) so override-name generation also consumes registry truth. Keep the compat `family` alias mirror-only and do not reopen browser/backend split authority while closing the remaining residuals. | No live backend bundle/session/export/runtime path still takes authority from `family` or `ENABLED_FAMILIES`; browser and backend both consume the same normalized contract; classic/runtime override naming also derives from registry `ahsw_range` instead of hardcoded maps | Any fix restores browser-side fail-close logic, creates a second registry authority, reintroduces live compat-family gating, or claims `UQ-004` closure while the hardcoded `ahsw_range` maps remain authoritative | Section 2.5 / `S2-R1` / `S2-R2` |
