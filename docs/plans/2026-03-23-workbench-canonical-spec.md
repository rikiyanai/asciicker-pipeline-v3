...
| Mounted-family contract now exists, but mounted-family authoring/runtime parity is still absent | `config/template_registry.json`, `src/pipeline_v2/service.py:1793-1812`, `scripts/xp_fidelity_test/bundle_contract.mjs`, `tests/xp_fidelity_test/semantic_runtime_contract.test.mjs`, `asciicker-Y9-2/engine/game.cpp`, `web/workbench.html:301-326`, `web/workbench.js:5760-5854` | `wolfie` and `wolack` are now represented explicitly in `prefix_catalog` with `runtime_role`, `mounted=true`, and blocker metadata. The remaining gap is execution: there are still no mounted presentation targets, no native builder for those families, no proposal-first semantic review loop that promotes only human-confirmed exact-cell truth, and no non-destructive workbench overlay-calibration surface for visually solving rider/mount alignment. The existing jitter controls mutate frame contents in place; they are not a mounted calibration owner. |
...
| `S2-R9` | `UQ-008` | mounted authoring/runtime proof | Add mounted presentation-target surfaces plus builder/export/runtime proof for `wolfie` and `wolack`, and land the two mounted authoring aids this queue row depends on: a proposal-first exact-cell semantic matcher/reviewer flow that stays gated by human confirmation, and a non-destructive workbench overlay-calibration mode that records visual `(dx,dy)` solves without shifting stored frame contents | Mounted rows are authorable and proven on the live contract, and mounted semantic/alignment authoring no longer depends on opaque manual notes or destructive jitter edits |
...
| UQ-008 | BLOCKED | Extend Section 2 to mounted-family authoring and runtime parity | UQ-007 pass condition met | Execute `S2-R9`: on top of the normalized registry and runtime identity layer, add mounted-family authoring/runtime parity for `wolfie` and `wolack`: mounted presentation-target surface, native builder support, export/runtime proof, mounted semantic-row closure, a proposal-first exact-cell matcher/reviewer flow that only promotes human-confirmed mounted semantics, and a whole-sheet workbench overlay-calibration mode for rider/mount visual alignment. That calibration surface must be non-destructive and artifact-producing: it records candidate/accepted `(dx,dy)` solves and overlay evidence without mutating stored XP art. Keep `bigbee` explicitly deferred unless canon changes. | `wolfie` and `wolack` are authorable/provable on the live Section 2 contract, mounted semantic/alignment truth is reviewable rather than implied, and mounted rows are no longer “specified_not_authorable” | Any fix pulls `bigbee` into scope without canon change, treats the current in-place jitter editor as mounted-calibration closure, writes mounted semantics directly from heuristics without human confirmation, or adds mounted support via a proof-only shim instead of the live authoring/runtime path | Section 2.5 / `S2-R9` |
